...
if (isset($post->deliver)) {
  ...
  $selectedItems = (isset($post->iid) && is_array($post->iid)) ? array_keys($post->iid) : [];
  if (empty($selectedItems)) {
    echo "<script>alert('Please select at least one item before submitting.');window.history.back();</script>";
    exit;
  }
  foreach ($selectedItems as $iid) {
    ...
    update(
      'invoice_item',
      "delivered=quantity, delivered_by=" . uid() . ", delivered_at=NOW(), delivery_staff='$deliveryStaffSql'",
      "id=$iid"
    );

    insert(
      'invoice_item_delviery',
      '`invoice_item_id`, `quantity`, `delivered_by`, `delivered_at`, delivery_staff',
      "$iid, $remainingQty, " . uid() . ", NOW(), '$deliveryStaffSql'"
    );
  }
    ...
    <div class="text-center mb-3-3 position-relative" style="background: linear-gradient(135deg, #1e90ff 0%, #00bfff 100%); padding: 20px 15px; margin: -8px -8px 0px -8px; border-radius: 0 0 20px 20px; box-shadow: 0 4px 12px rgba(30, 144, 255, 0.3);">
      ...
      <div style="position: absolute; top: 15px; right: 15px; text-align:right;">
        <?php if ($isDeliveryStaff) { ?>
          ...
          <select id="delivery-staff-filter" class="form-select form-select-sm" style="min-width: 70px; margin-top: 6px; font-size: 0.8rem; color: #fff; padding: 4px 0 4px 4px; display:none;">
        ...
        <?php } else { ?>
          <select id="delivery-staff-filter" class="form-select form-select-sm" style="min-width: 70px; margin-top: 6px; font-size: 0.8rem;">
            ...
            while ($staff = mysqli_fetch_object($deliveryStaffFilter)) {
      ...
      $("#update_quantity_button").click(function() {
        ...
        $.post('/ajax/update_invoice_item_quantity.php', {
          ...
          .done((response) => {
            ...
            $('#invoice-item-price-' + invoice_item_id).text(total.toFixed(2));
            var myModal = bootstrap.Modal.getInstance(document.getElementById('modal-modify-quantity'));
...
<script>
  (function(){
    // Intercept action-bar button clicks (Deliver/Collect)
    document.addEventListener('click', function(e){
      var btn = e.target && e.target.closest && e.target.closest('#btnDeliver,#btnCollect');
      if (!btn) return;

      var form = document.getElementById('dcollectForm');
      if (!form) return;

      // Count only real item checkboxes; ignore consolidated summary checkboxes
    ...
    // Intercept form submit (works even if submission triggered programmatically)
