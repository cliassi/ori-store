...
<form method='post' action='/store/ajax/dcollect_delivery.php'>
  <div class="customer-container">
    ...
    while ($customer = mysqli_fetch_object($customers)) {
      ...
      if ($pending) {
        ...
        $filter .= " ii.delivered < ii.quantity AND IFNULL(ii.delivery_date,i.invoice_date) <= curdate()";
      } else {
        if ($filter) {
          $filter .= " AND ";
        }
      ...
      while ($i = mysqli_fetch_object($items)) {
        ...
        if ($i->quantity > 0) {
          ...
          if ($delivery) {
            ...
            print "</td><td class='text-center order-qty-col'>" . ($restrictedUser ? "<span>$collected</span>" : "<a href='#' id='invoice-item-$i->iid' data-bs-toggle='modal' onClick='setItemId($i->iid)' data-bs-target='#modal-modify-quantity'>$collected</a>") . "</td>";
          }
          print "</tr>";
          $ic++;
        }
