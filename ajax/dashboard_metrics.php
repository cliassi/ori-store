...
// dateSelector outputs YYYY-MM-DD, normalize to YYYY-MM
if ($mon && preg_match('/^\d{4}-\d{2}-\d{2}$/', $mon)) {
  $mon = substr($mon, 0, 7);
}
    ...
    <table id="customer-table" class="table table-striped table-bordered nowrap">
      <tbody>
        ...
        <tr>
          ...
          <td>:</td>
          <td width="250px" title="<?php print "$store_value->amount - $damage->amount - $sreturn->amount"; ?>">
        ...
        <tr>
          ...
          <td>:</td>
          <td><span style="font-weight: 300"><?php print '(' . nf($store_income) . ')'; ?></span>
            <?php print nf($store_income - $store_expense); ?></td>
        </tr>
      </tbody>
    </table>
  </div>
