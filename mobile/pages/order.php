...
if(isset($post->assign) && nn($post->delivery_staff)){
  // For Delivery Staff, force assignment to the logged-in staff name
  $deliveryStaff = $isDeliveryStaff && nn($loggedStaffName)
    ? trim($loggedStaffName)
    : trim($post->delivery_staff);
  $selectedIids = [];
    ...
    if ($selectedCustomerIds) {
      ...
      $fallbackQuery = "SELECT ii.id
              ...
              WHERE i.customer_id IN ($customerIdsSql)
                ...
                AND (ii.assigned_at IS NULL OR ii.assigned_to IS NULL OR ii.assigned_to = '')";
  ...
  if ($selectedIids) {
    redir("?page=order&uid=" . uid() . "&assigned=1");
  } else {
    redir("?page=order&uid=" . uid());
  }
  exit;
}
?>
    ...
    <style type="text/css">
      ...
      a.has-checkbox {
        ...
        color: #000;
                ...
                <?php  print $button; ?>
    <div class="text-center mb-3-3" style="background: linear-gradient(135deg, #1e90ff 0%, #00bfff 100%); padding: 20px 15px; margin: -8px -8px 0px -8px; border-radius: 0 0 20px 20px; box-shadow: 0 4px 12px rgba(30, 144, 255, 0.3);">
      <h1 class="h3 mb-0" style="color: white; font-weight: 600; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">Order List</h1>
    </div>
    ...
    justify-content: space-between;">
      ...
      <button id="btnCollect" class="btn btn-warning" type="submit" name="assign" value="1" form="dcollectForm" style="flex: 1; max-width: 110px; font-size:.75rem; margin-left:auto;">Assign</button>
...
<script>
  (function(){
    // Intercept action-bar button clicks (Assign/Deliver)
    document.addEventListener('click', function(e){
      var btn = e.target && e.target.closest && e.target.closest('#btnCollect,#btnDeliver');
      if (!btn) return;

      var form = document.getElementById('dcollectForm');
      if (!form) return;

      // Only count real item checkboxes; ignore consolidated summary
      var anyChecked = form.querySelectorAll(
        "input.iid-date:not(.consolidated-toggle):checked, " +
        "#cust-body-0 input.iid-date:checked" // keep selector simple; adjust if needed
      ).length > 0;
    ...
    document.addEventListener('submit', function(e){
      ...
      var anyChecked = f.querySelectorAll('input.iid-date:not(.consolidated-toggle):checked').length > 0;
      if (!anyChecked) {
        e.preventDefault();
        if (typeof Swal !== 'undefined') {
          Swal.fire({ icon: 'warning', title: 'No items selected', text: 'Please select at least one item before submitting.' });
