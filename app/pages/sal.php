...
if(isset($post->save_worker_payment)){
	...
	// var_dump($statement);
	// vd($post);
	$salary = ($statement->type == 'Parttime' ? round($w->basic * $w->working_days) : round($w->basic / 26 * $w->working_days)) + ($w->working_days > 25 ? 100 : 0);
...
print "<br><br>";
print "<a class='pointer btn btn-primary' data-toggle='modal' data-target='#modal-create-fulltime'>Create Fulltime Hotel Statement</a>".space(2);
print "<a class='pointer btn btn-success' data-toggle='modal' data-target='#modal-create-parttime'>Create Parttime Hotel Statement</a>".space(2);
print "<a class='pointer btn btn-warning' href='?page=17' style='background: violet; color: #000'>Parttime Working Sheet</a>".space(2);
print "<a class='pointer btn btn-warning' href='?page=14'>Hotel Payment</a>".space(2);
print "<a class='pointer btn btn-danger' href='?page=15'>Card Holder</a>".space(2);
print "<a class='pointer btn btn-info' href='?page=13'>House & Hostel</a>";
...
print "<span style='float:right'>
	<a class='pointer btn btn-default'  href='?page=3&mon=". subMonth(1, $date)."$bt_url'><i class='fa fa-chevron-left'></i>Prev</a>".space(5);
	//."<b>";
// print date("M Y", strtotime($mon."-01"))."</b>";
print monthSelector('mon', date("Y-m-d", strtotime($mon."-01")));
print space(5).
	"<a class='pointer btn btn-default' href='?page=3&mon=". addMonth(1, $date)."$bt_url'>Next <i class='fa fa-chevron-right'></i></a>
	</span>";
print "<br><br>";
...
if(isset($get->h)){
	$hotel = R::load("hotel", $hotel_statement->hotel);
	print "<h1 class='center panel panel-success'><span id='title-hotel'>$hotel->name</span> Salary Statement - <span id='title-month'>".date("M Y", strtotime("{$hotel_statement->month}-01"))."</span></h1>";
	...
	print "
		<table class='table table-bordered table-striped'>
			<thead>
				<tr>
					...
					<th>No</th>
					<th>".(isUserIn(['orange']) ? "<i class='fa fa-shopping-cart pointer' data-toggle='modal' data-target='#modal-worker-payment-2' onClick='setWorkerIds()'></i>" : "")."</th>
					<th>Name</th>
					<th>Hotel Salary</th>
					<th>Basic Salary</th>
		...
		while($w = mysqli_fetch_object($workers)){
			...
			if($hotel_statement->type == 'Fulltime'){
				$salary = round(($w->basic / 26 * ($w->working_days + $w->public_holiday)) + ($w->working_days > 25 ? 100 : 0));
			} else{
			...
			print "<tr>
					...
					<td>
						<a href='#' onClick='showDetails($w->statement, $w->id,\"$w->name\", \"".nf0($salary)."\", \"".nf0($income)."\", \"".nf0($salary + $income)."\",\"$w->phone\",\"$w->account\",$w->lock)'>$w->name</a>
						<button class='btn btn-$w->category frht' style='' ".(isUserIn(['superadmin']) ? "onClick='setCategory($w->id)'" : '').">$w->category</button>
						<div>$w->phone</div>
					</td>
					<td class='text-center'>".nf0($w->billed_amount)."</td>
					<td class='text-center'>".nf0($w->basic)."</td>
					<td class='text-center' ondblclick='editWorkingDays($w->id)' data-data='$working_days' id='w{$w->id}'>$w->working_days ";
					if($w->working_hours){
						print " + {$w->working_hours}HR";
					}
					if($w->public_holiday){
						print " + {$w->public_holiday}PH";
					}
					...
					<td class='text-center'>".nf0($salary + $income - $w->paid)."</td>
					...
					if(isUserIn(['superadmin', 'orange', 'apple'])){
						print "<td class='text-center'><a data-toggle='modal' data-target='#modal-worker-edit' onClick='setWorkerId($w->id)' class='btn btn-sm btn-warning'><i class='fa fa-edit'></i></a></td>";
					}
					if(isUserIn(['superadmin', 'orange', 'apple'])){
						print "<td class='text-center' nowrap>
							<a data-toggle='modal' data-target='#modal-deduct-salary' onClick='setWorkerId($w->id)' class='btn btn-sm btn-danger'><i class='fa fa-minus'></i></a>
							<a data-toggle='modal' data-target='#modal-add-salary' onClick='setWorkerId($w->id)' class='btn btn-sm btn-warning'><i class='fa fa-plus'></i></a>
						</td>";
					}
					if(isUserIn(['superadmin'])){
	...
	print "
			...
			<tfoot>
				<tr>
					<td></td>
					<td colspan='3'><a data-toggle='modal' data-target='#modal-worker'><i class='fa fa-plus'></i></a> <b class='frht style='color:teal;'>Hotel Invoice = ".nf0($hotel_salary * sum('working_days'))."</b></td>
					<td></td>
					<th class='text-center'>".nf0(sum('basic'))."</th>
					<th class='text-center'>".nf0(sum('working_days'))."</th>
					<td></td>
	...
	print "<div style='padding: 5px 15px; font-weight:700; font-size:1.5rem; margin-bottom:10px; border-radius; background-color: ".(nn($hotel_statement->background_color) ? $hotel_statement->background_color : "#efefef")."; color: $hotel_statement->text_color'>$hotel_statement->remarks <a data-toggle='modal' data-target='#save_remarks_modal'><i class='fa fa-plus-circle'></i></a></div>";
	...
	if(isset($get->statement)){
	  ...
	  print makeSelectOption("name='method' class='form-control-fluid'", ['All', 'Income', 'Capital', 'Expense', 'Withdraw'], ['', 'Income', 'Capital', 'Expense', 'Withdraw'], $method);
		...
		print "<tr><th colspan='12'><h3>Pinjam</h3></tr>";
		$rows = select("SELECT * FROM (
			SELECT id, expense_date date, amount, particulars, entry_time, '' status, 'expense_account_entry' source FROM `expense_account_entry` WHERE particulars LIKE '%pinjam%' AND entry_id IN (SELECT id FROM hotel_statement_worker_payment WHERE worker IN (SELECT id FROM hotel_statement_worker WHERE statement=$get->h))
		) t ORDER BY entry_time");
		...
		$rows = select("SELECT * FROM (
			...
			UNION
			SELECT e.id, e.expense_date date, e.amount, e.particulars, e.entry_time, `status`, 'expense_account_entry' source FROM expense_account_entry e, expense_account a WHERE e.accountid=a.id AND a.hotel=$stt->hotel AND e.expense_date LIKE '{$stt->month}-%' AND e.particulars NOT LIKE '%pinjam%' AND e.particulars NOT LIKE '%salary%'
		) t ORDER BY entry_time");
		...
		$expense = getSum("expense_account_entry", "amount", "accountpath LIKE '%/$hotel->accountid/%' AND month='{$stt->month}' AND tran_type='Debit' AND particulars NOT LIKE '%salary%' AND particulars NOT LIKE '%pinjam%'");
		...
		print "<div class='col-md-6 right'>
			<!--a class='pointer w100 btn btn-warning'  data-toggle='modal' data-target='#modal-capital'>PSI</a-->
			";
		if(($hotel_statement->invoice)){
			...
			print "<div class='dropdown' style='display:inline-block'>
				  <button class='btn btn-success dropdown-toggle' type='button' data-toggle='dropdown'>Print Hotel Invoice
			...
			if($hotel_statement->type == 'Fulltime'){
				...
				print "<div class='dropdown' style='display:inline-block'>
				  <button class='btn btn-info dropdown-toggle' type='button' data-toggle='dropdown'>Preview Invoice
				  <span class='caret'></span></button>
				  <ul class='dropdown-menu'>
			...
			if(isUserIn(['superadmin', 'orange'])){
				print "<a class='pointer w160 btn btn-warning'  data-toggle='modal' data-target='#modal-hotel-invoice'>Create Hotel Invoice</a>";
			}
		} 
		print "<a class='pointer w100 btn btn-primary' data-toggle='modal' data-target='#modal-income'>Income</a>
			<a class='pointer w100 btn btn-danger' id='btn-expense' href='/app/expense_account/hotel?hotel=$hotel->id&month=$mon' data-toggle='modal--' data-target='#modal-expense--'>Expense</a>
			<!--a class='pointer w100 btn btn-info'  data-toggle='modal' data-target='#modal-withdraw'>Withdraw</a-->";
		print "</div>";
	...
	while($hotel = mysqli_fetch_object($hotels)){
		...
		while($w = mysqli_fetch_object($workers)){
			if($hotel_statement->type == 'Fulltime'){
				$salary = round(($w->basic / 26 * $w->working_days) + ($w->working_days > 25 ? 100 : 0));
			} else{
				$salary = $w->basic * $w->working_days;
			}
			$total_salary += $salary;
		...
		// $expense = getSum("hotel_expense", "amount", "statement=$hotel->id");
		$salary = getSum("expense_account_entry", "amount", "accountpath LIKE '%/$hotel->haid/%' AND month='{$hotel_statement->month}' AND tran_type='Debit' AND (particulars LIKE '%salary%' OR particulars LIKE '%pinjam%')");
		$expense = getSum("expense_account_entry", "amount", "accountpath LIKE '%/$hotel->haid/%' AND month='{$hotel_statement->month}' AND tran_type='Debit' AND particulars NOT LIKE '%salary%' AND particulars NOT LIKE '%pinjam%'");
		...
		print "<td class='text-center'>".nf0($salary)."</td>"; sum("salary", $salary);
		print "<td class='text-center'>".nf0($expense)."</td>"; sum("expense", $expense);
	...
	print "<tr>
		<tfoot>
			...
			<th>".nf0(sum("income"))."</th>
			<th>".nf0(sum("salary"))."</th>
			<th>".nf0(sum("expense"))."</th>
			<th>".nf0(sum("profit"))."</th>
			<th>".nf0(sum("pending"))."</th>
			<th>".nf0(sum("pending_expense"))."</th>
			<th></th>
		</tfoot>
	</tr>";
	print "</table>";
    ...
    <div class="modal-content">
    	<form method="post" autocomplete="off" enctype='multipart/form-data'>
	      ...
	      <div class="modal-header">
	        ...
	        <h4 class="modal-title">Create Fulltime Hotel Statement</h4>
	      </div>
	      <div class="modal-body">
	      	<table>
	      		<tr><td>Hotel</td><td><?php print sop2('hotel'); ?>  <a data-toggle='modal' data-target='#modal-hotel'><i class='fa fa-plus'></i></a></td></tr>
  ...
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
    	<form method="post" autocomplete="off" enctype='multipart/form-data'>
	      ...
	      <div class="modal-body">
	      	<table>
	      		<tr><td>Hotel</td><td><input name="name" required class="form-control" placeholder="Hotel name"></td></tr>
	      		<tr><td>Basic Salary</td><td><input name="basic" type='number' required class="form-control" placeholder="Basic Salary"></td></tr>
	      	</table>
	      </div>
	      <div class="modal-footer">
    ...
    <div class="modal-content">
    	<form method="post" autocomplete="off" enctype='multipart/form-data'>
	      ...
	      <div class="modal-body">
	      	<table>
	      		<tr><td>Name</td><td><input name="name" class="form-control" placeholder="Name"></td></tr>
	      		<tr><td>Hotel Salary</td><td><input type="number" name="billed_amount" class="form-control" placeholder="Hotel Salary"></td></tr>
