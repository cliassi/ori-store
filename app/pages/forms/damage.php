...
if(isset($post->save)){
  foreach ($post->variance as $id => $qty) {
    ...
    $ii->quantity = $qty;
    $ii->price = $variance->price / $variance->unit;
    $ii->cost = $variance->cost / $variance->unit;
...
<style type="text/css">
            ...
            <?php endif; ?>
                        ...
                        <tbody>
                          ...
                          while ($product = mysqli_fetch_object($products)) {
                            $pc = R::load("product_category", $product->product_category_id);
                            $variances = R::find("product_variance", "image_single IS NOT NULL AND product_id=?", [$product->id]);
                            if(count($variances) == 0) continue;
                            print "<tr>";
                  ...
                  <!-- <div class="col-12">
                    <div class="order-total ms-auto">
                      <div class="row">
                        ...
                        <div class="col-6"> <p class="f-w-600 mb-1 grand-total">RM <?php print nf($total); ?></p></div>
                      </div>
                    </div>
                  </div> -->
                  <!-- <div class="col-12">
  ...
  <script type="text/javascript">
    $(".supplier-select").change(function(){
      const id = $(this).val();
      // location.href = "?supplier=" + id + "<?php if(isset($get->v) && isset($get->qty)) print "&v=$get->v&qty=$get->qty"; ?>";
    });
