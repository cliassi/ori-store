...
"`stock_collect_item` ii, `product_variance` pv, product p", "pv.id=ii.product_variance_id AND p.id=pv.product_id AND stock_collect_id=$tran->id");
  ...
  print "<td class='text-center'><div class='cta-container'><span class='ctas'>".($tran->src == 'invoice' ? 'INV' : 'STC').zerofill($tran->id, 5)."</span>
  <div class='cta-menu'><a href='?s={$get->s}&del&id=$tran->id' class='protected-link'><i class='fas fa-trash-alt'></i></a>&nbsp;&nbsp;
  <a href='?s={$get->s}&edit=$tran->id' ><i class='fas fa-edit'></i></a></div>
  </div></td>";
  $particular = $collect = $delivered = $returened = $pending = $damaged = $balance = "";
  while ($item = mysqli_fetch_object($stock)) {
    ...
    if(isset($get->edit) && $get->edit == $tran->id){
      $returened .= "<div class='order-item'><input type='number' class='w50' value='$item->returned_quantity' name='ri[$item->id]'> <span class='damage-item hidden' title='$item->damaged_cause'>".($item->damaged_quantity ? $item->damaged_quantity : '&nbsp;')."</span> <span class='pad-right-5'></span><a data-bs-toggle='modal' data-bs-target='#stockReturn' onClick='setId($item->id)' data-quantity class='btn btn-return btn-sm btn-".($item->returned_quantity==0?'warning':'success')."' data-id='$item->id'><i class='fas fa-people-carry'></i></a></div>";
    } else{
      $returened .= "<div class='order-item'><b class='damage-item'>$item->returned_quantity</b> <span class='damage-item hidden' title='$item->damaged_cause'>".($item->damaged_quantity ? $item->damaged_quantity : '&nbsp;')."</span> <span class='pad-right-5'></span><a data-bs-toggle='modal' data-bs-target='#stockReturn' onClick='setId($item->id)' data-quantity class='btn btn-return btn-sm btn-".($item->returned_quantity==0?'warning':'success')."' data-id='$item->id'><i class='fas fa-people-carry'></i></a></div>";
    }
    ...
    $balance .= "<div class='order-item' title='$item->damaged_cause'><b class='damage-item'>".($item->quantity - ($item->delivered + $item->returned_quantity + $item->damaged_quantity))."</b></div>";
  ...
  if(isset($get->edit) && $get->edit == $tran->id){
    $returened = "<form method='post'>$returened<br><div><button class='btn btn-sm btn-danger' name='update_returned'>Update</button></form>";
  }
  ...
  print "<td class='text-center'>$balance</td>";
...
print "</tbody>";
          ...
          <?php
            ...
            <tr>
            <th class='w100'>No</th>
            <th>Collection & Return Report</th>
