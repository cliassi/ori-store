...
if(isset($post->delete_outsource_expense)){
  ...
  print "Date ".dp('d', $d, uid()!=1 ? prevDay() : false)." - ".dp('t', $t, uid()!=1 ? prevDay() : false);
...
<table class="table table-bordered">
	...
	<tr><th rowspan="2">No.</th><th rowspan="2">Particulars</th><th rowspan="2">Name</th><th rowspan="2">Company</th><th rowspan="2">Entry By</th><th colspan="3" class='center'>Cash</th><th colspan="3" class='center'>Bank</th></tr>
  <tr><th>Credit</th><th>Debit</th><th>Balance</th><th>Credit</th><th>Debit</th><th>Balance</th></tr>
  ...
  <?php
    ...
    if($_collection){
      $official_receipts = select("o.*, w.contact name, w.company, w.id wid", "collection o, customer w", "(w.branch_id = $branch_id OR w.branch_id IS NULL) AND o.customer_id=w.id AND o.deleted_by IS NULL AND (o.date BETWEEN '$d 00:00:00' AND '$t 23:59:59')");
      while($official_receipt = mysqli_fetch_object($official_receipts)){
        ...
        if($official_receipt->payment_method == 'Cash'){
          addTran($_trans, stripslashes($official_receipt->description), "<a href='/store/customer/statement/$official_receipt->wid' target='_blank'>$official_receipt->name</a>", '', $official_receipt->company, $avatar,$official_receipt->amount,'','','','','');
        } else{
          addTran($_trans, stripslashes($official_receipt->description), "<a href='$/store/customer/statement/$official_receipt->wid' target='_blank'>$official_receipt->name</a>", '', $official_receipt->company, '','','',$avatar,$official_receipt->amount,'','');
        }
        print "<tr title='or'>
          <td>$i</td>
          <td";
        if(strpos($official_receipt->description, 'Hotel salary theke permit') !== FALSE){
          print " style='background:#00ff0033'";
        }
        print ">".stripslashes($official_receipt->description)."</td>
...
// if(isUserIn(['apple', 'superadmin', 'orange'])){
  ...
  if(isUserIn(["lemon","superadmin","ebrahim"])){
