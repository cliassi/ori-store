...
if(isset($get->cw)){
	...
	print "<h3>Outlet Account Petty Cash Report</h3>";
	...
	$summary2 = mysqli_fetch_object(select("SELECT 
		(SELECT SUM(IFNULL(amount,0)) FROM `cw_cash` WHERE (branch_id = $branch_id OR branch_id IS NULL) AND entry_time<'$d 00:00:00') add_cash, 
		(SELECT SUM(IFNULL(amount,0)) FROM `bd_handover` WHERE (branch_id = $branch_id OR branch_id IS NULL) AND created_at<'$d 00:00:00') cash_handover, 
		(SELECT SUM(IFNULL(amount,0)) FROM `expense_account_entry` WHERE (branch_id = $branch_id OR branch_id IS NULL) AND payment_method='Cash' AND expense_date<'$d 00:00:00') cash_expense, 
		(SELECT SUM(IFNULL(amount,0)) FROM `payment` WHERE (branch_id = $branch_id OR branch_id IS NULL) AND payment_method='Cash' AND created_at<'$d 00:00:00') cash_payment, 
		(SELECT IFNULL(SUM(IFNULL(amount,0)),0) FROM `cw_cash` WHERE (branch_id = $branch_id OR branch_id IS NULL) AND amount>0  AND entry_time<'$d 00:00:00') cash, 
		(SELECT SUM(IFNULL(amount,0)) FROM `cw_cash_withdraw` WHERE (branch_id = $branch_id OR branch_id IS NULL) AND entry_time<'$d 00:00:00') withdraw"));

	$total = $handover_cash - $expense_entry + $withdraw;
    ...
    $query = "SELECT * FROM (
			SELECT '' se, 'cw_outlet' source, id, amount, `date`, entry_time, entry_by, particulars, `status`, '' ref, '' done_by, '' done_time, 0 checked FROM cw_outlet WHERE  branch_id=$branch_id AND (date BETWEEN '$d' AND '$t')
		) t ORDER BY date";
	$trans = select($query);
...
print "<form method='post'>";
	...
	while($tr = mysqli_fetch_object($trans)){
		...
		if($tr->source == 'bd_handover'){
			...
			$total += $tr->amount;
		} elseif($tr->source == 'cw_cash' || $tr->source == 'cw_outlet'){
			sum('cash', $tr->amount);
