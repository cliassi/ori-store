        ...
        <div class="dt-responsive table-responsive">
          <table id="simpletable" class="table table-striped table-bordered nowrap">
            ...
            <tbody>
              ...
              $i = 1;
              if(defined('ID')){
                $items = select("SELECT v.id, p.id pid, p.name pname, p.image pimage, v.particulars, v.cost, v.price, v.image vimage, size, unit, oi.quantity, order_date FROM `product` p, `product_variance` v, `goods_return_item` oi, `goods_return` o WHERE p.id=v.product_id AND v.id=oi.product_variance_id AND o.id=oi.order_id AND v.id=".ID." ORDER BY order_date");
                while($var = mysqli_fetch_object($items)){
                    print "<tr id='var-'>";
                    print "<td>$i</td>";
                    print "<td id='particulars-$var->id'>".df($var->order_date)."</td>";
                ...
                $items = select("SELECT v.id, p.id pid, p.name pname, p.image pimage, v.particulars, v.cost, v.price, v.image vimage, size, unit, SUM(oi.quantity) quantity, order_date FROM `product` p, `product_variance` v, `goods_return_item` oi, `goods_return` o WHERE p.id=v.product_id AND v.id=oi.product_variance_id AND o.id=oi.order_id GROUP BY v.id ORDER BY order_date");
                while($var = mysqli_fetch_object($items)){
                    print "<tr id='var-'>";
                    print "<td>$i</td>";
                    print "<td id='particulars-$var->id'>".df($var->order_date)."</td>";
