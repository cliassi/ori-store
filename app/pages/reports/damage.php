...
if(uid()==1 && isset($get->del)){
  if(isset($get->conf)){  
    $object = R::load('damaged_item', $get->del);
    // vd($object);
    // $object->trash = 1;
    // R::store($object);
    R::trash($object);
    ...
    <script type="text/javascript">
      if(prompt("Please key in your PIN to remove this?") == "<?php print upin(); ?>"){
        location.href = "<?php print "?del=$get->del&conf"; ?>";
      } else{
        alert("Wrong PIN entered!");
        location.href = "<?php print "?"; ?>";
      }
        ...
        <div class="dt-responsive table-responsive">
          <table id="simpletable" class="table table-striped table-bordered nowrap">
            ...
            <tbody>
              <?php 
              $i = 1;
                $items = select("SELECT oi.id damaged_item_id, v.id, p.id pid, p.name pname, p.image pimage, v.particulars, v.cost, v.price, v.image_single vimage, size, unit, oi.quantity, oi.created_at FROM `product` p, `product_variance` v, `damaged_item` oi WHERE p.id=v.product_id AND v.id=oi.product_variance_id  ORDER BY oi.created_at");
                while($var = mysqli_fetch_object($items)){
                    print "<tr id='var-'>";
                    print "<td>$i</td>";
                    print "<td id='particulars-$var->id'>".df($var->created_at)."</td>";
                    ...
                    print "<td id='quantity-$var->id'>$var->quantity Pcs<t/d>";
                    ...
                    print "<td><a href='?del=$var->damaged_item_id' ><i class='fa fa-trash'></i></a></td>";
                    print "</tr>";
                    $i++;
                }
