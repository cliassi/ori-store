...
.sticky-table-container table {
    ...
    font-size: 0.825rem;
    min-width: 800px; /* Prevents table from being too compressed */
}
...
.bg-purchase { background-color: rgba(25, 135, 84, 0.1) !important; }
.bg-collection { background-color: rgba(220, 53, 69, 0.1) !important; }
.bg-return { background-color: rgba(13, 110, 253, 0.1) !important; }
.bg-damage { background-color: rgba(255, 193, 7, 0.1) !important; }
.qty-cell-inline {
    display: inline-flex;
    align-items: center;
}
...
<?php if (defined('ID')): ?>
    <form class="mb-3" method="get" action="" style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
        ...
        <input type="date" class="form-control" name="from_date" 
               value="<?php echo htmlspecialchars($form_from_date); ?>" 
                                ...
                                if (defined('ID')) {
                                    ...
                                    $sql = "
                                        SELECT * FROM (
                                            ...
                                            oi.quantity quantity, o.order_date `date`, oi.created_at, oi.created_by, '' staff, 'Warehouse' `remarks`, '' status, NULL AS customer_id, NULL AS customer_code,
                                    ...
                                    $items = select("SELECT id, pid, pname, pimage, particulars, cost, price, vimage, size, unit, SUM(quantity) quantity, `date` FROM (
                                        ...
                                        UNION
                                        SELECT oi.id iid, v.id, p.id pid, p.name pname, p.image pimage, v.particulars, v.cost, v.price, v.image vimage, size, unit, oi.quantity, order_date `date`
                                        ...
                                        UNION
                                        SELECT oi.id iid, v.id, p.id pid, p.name pname, p.image pimage, v.particulars, v.cost, v.price, v.image vimage, size, unit, oi.returned_quantity quantity, DATE `date`
                                        ...
                                        UNION
                                        SELECT oi.id iid, v.id, p.id pid, p.name pname, p.image pimage, v.particulars, v.cost, v.price, v.image vimage, size, unit, -oi.damaged_quantity quantity, DATE `date`
                                        FROM `product` p, `product_variance` v, `stock_collect_item` oi, `stock_collect` o WHERE p.id=v.product_id 
