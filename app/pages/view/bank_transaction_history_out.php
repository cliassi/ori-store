...
if(isUserIn($uaccs) && isset($get->unlink)){
	if(isset($get->conf)){		
...
openForm();
	...
	// }
	print "<th style='max-width: 500px; width: 500px;'>Particulars</th><th></th><th class='w100'>Amount</th><th></th><th></th><th class='w150'>Status</th>";
	// if(isUserIn($uaccs)){
	...
	while($t = mysqli_fetch_object($bank_transactions)){
		print "<tr><td class='w40'><input type='hidden' name='trans[]' value='{$t->id}'><a href='view/$t->id'>$i</a></td>
		...
		// if($tab == 'i'){
			...
			// }
		// }
		print "<td".(uid()==1?' class="tid-'.$t->id.'" ondblclick="javascript:edit('.$t->id.',\''.$t->particulars.'\')"':'').">{$t->particulars}";
		...
		if(!nn($t->particulars) && isUserIn($uaccs)){
			...
			print "<a class='pointer btn btn-primary' data-toggle='modal' data-target='#modal-expense' onclick='setId($t->id)'><i class='fa fa-chat'></i></a></td><td>";
...
<script type="text/javascript">
	...
	function toggleCheck(id) {
		var value = $("#bti-" + id).data("value"); 

		$.post("/app/ajax/toggle_bank_check.php", {id:id,value:value == "1" ? "0" : "1"}, function(data){
			$("#bti-" + id).data("value", value == "1" ? "0" : "1");
			$("#bti-" + id).html(data);
		});	
	}
	...
	function approve(i){
		...
		if(confirm("Are you sure you want to approve this?")){
			location.href = "?approve=" + i + "&url=<?php $url = $_SERVER['QUERY_STRING']; $url = strpos($url, '&') === FALSE ? "" : substr($url, strposX($url, "&", 1) + 1); print base64_encode($url); ?>";
		}
		<?php endif; ?>
	}
	...
	function edit(i,txt){
		$(".tid-" + i).html("<input class='form-control' name='particulars_"+i+"' value='"+txt+"'>");
	}
