...
while ($item = mysqli_fetch_object($trans)) {
  ...
  if ($item->src == 'invoice') {
    ...
    print "<td title='$oi->delivered_by'><button title='$oi->delivered_by " . (nn($oi->delivered_at) ? ' @ ' . df($oi->delivered_at) : '') . "' class='btn btn-sm " . ($oi->delivered_by ? "btn-success" : "btn-warning") . " selected-delivery-man' type='button' aria-expanded='false'><i class='fas fa-shipping-fast'></i> " . ($oi->delivered_by ? "$oi->delivery_staff" : "") . "<span></span></button></td>";
...
print "</td>
<th class='text-right' colspan='4'><button name='deliver' class='btn btn-sm btn-success hidden'>Submit</button> TOTAL</th>
<th></th>
