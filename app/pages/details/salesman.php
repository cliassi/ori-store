...
<?php
$obj = R::dispense('salesman');
if (defined('ID')) {
    $obj = R::load('salesman', ID);
}

if(isset($post->idToDelete)){
  $inv = R::load("stock_collect", $post->idToDelete);
  $ret = R::find("stock_return", "stock_collect_id=?", [$inv->id]);
  if(count($ret) > 0){
    print "<div class='alert alert-danger'>You cannot delete this <b>stock collect</b>, remove the <b>stock return</b>s first.</div>";
  } else{
    del("stock_collect_item", "stock_collect_id='$inv->id'");
    R::trash($inv);
  }
}
if(isset($post->idToDelete2)){
  $col = R::load("stock_return", $post->idToDelete2);
  $items = R::find("stock_return_item", "stock_return_id=?", [$col->id]);
  foreach($items as $item){
    update("stock_collect_item", "returned_quantity = returned_quantity - $item->quantity", "id=$item->stock_collect_item_id");
  }
  del("stock_return_item", "stock_collect_id='$col->id'");
  R::trash($col);
}
...
if(isset($post->save_remarks)){
  $salesman_remarks = R::dispense("salesman_remarks");
  $salesman_remarks->salesman_id = $obj->id;
  $salesman_remarks->notes = $post->remarks;
  $salesman_remarks->priority = 'high';
  $salesman_remarks->entry_by = uid();
  R::store($salesman_remarks);
  redir("?");
}

if(isset($get->delivered)){
  $stock_collect = R::load('stock_collect', $get->delivered);
  $stock_collect->delivered_by = uid();
  $stock_collect->delivery_date = now();
  R::store($stock_collect);
  redir("?");
}
...
$trans = select("SELECT * FROM (SELECT * FROM (
  SELECT 'stock_collect' src, id, date, created_at, created_by, stockCollectItems(id) particulars, (SELECT SUM(quantity) FROM `stock_collect_item` ii WHERE ii.stock_collect_id=stock_collect.id) amount FROM `stock_collect` WHERE salesman_id=$obj->id
  UNION
  SELECT 'stock_return' src, id, date, created_at, created_by, stockReturnItems(id) particulars, (SELECT SUM(quantity) FROM `stock_return_item` ii WHERE ii.stock_return_id=stock_return.id) amount FROM `stock_return` WHERE salesman_id=$obj->id
...
while ($item = mysqli_fetch_object($trans)) {
  ...
  if($item->src == 'stock_collect'){
    print "<td>SC".zerofill($item->id, 5)."</td>";
  } else{
    print "<td>SR".zerofill($item->id, 5)."</td>";
  }
  print "<td>$item->particulars</td>";
  // if($item->src == 'stock_collect'){
  //   print "<td>".($item->created_by ? "<a class='btn btn-success'>Delivered</a>" : "<a class='btn btn-warning' href='?delivered=$item->id'>Ordered</a>")."</td>";
  // } elseif(strrpos($item->particulars, 'bank account') !== FALSE){
  //   print "<td>".($item->created_by ? "<a class='btn btn-success'>Approved</a>" : "<a class='btn btn-warning' href='?approved=$item->id'>Pending</a>")."</td>";
  // } else{
  if($item->src == 'stock_collect'){
    print "<td><a class='btn btn-danger' href='../return/".$item->id."'>Return</a></td>";
  } else{
    print "<td></td>";
  }
  // }
  ...
  print "<td class='text-right'>".(sum('balance'))."</td>";
  print "<td>";
  print "<a type='button' class='btn btn-sm btn-warning' href='".ROOT."/stock_collect/print/$item->id' ><i class='fas fa-print'></i></a><br>";
  print "<a type='button' class='btn btn-sm btn-info pb-1' href='".ROOT."/stock_collect/edit/$item->id' ><i class='fas fa-edit'></i></a><br>";
  if($item->src == 'stock_collect'){
...
<th class='text-right'>".(sum('debit'))."</th>
<th class='text-right'>".(sum('credit'))."</th>
<th class='text-right'>".(sum('balance'))."</th>
<th></th>
</tr>";
...
$remarks = select("*", "salesman_remarks", "salesman_id=$obj->id AND trash=0 AND entry_by=1", "ORDER BY id DESC");
