...
if (METHOD == 'add') {
...
} elseif (METHOD == 'supplierProduct') {
  require 'details/supplierProduct.php';
} else {
  if (isset($get->v)) {
    $product = R::load("product", $get->id);
  ...
  $fields = [
    ...
    "name" => ["label" => "Main Product Name", "display" => '', 'link' => 'details'],
    ...
    "visible" => ["label" => "Available?", "display" => '', 'type' => 'visible'],
                    ...
                    if (isset($value['display'])) {
                      ...
                      if (isset($value['type'])) {
                        if ($value['type'] == "visible") {
                          print "<td>" . ($obj->$key ? "<a href='?id=$obj->id&v=0'><i class='fas fa-eye'></i></a>" : "<a href='?id=$obj->id&v=1'><i class='fas fa-eye-slash'></i></a>") . "</td>";
                          $printed = true;
                        } else
                          if ($value['type'] == "image") {
                      ...
                      if (!$printed) {
                        print "<td>";
                        if (isset($value['link']))
                          print "<a href='$page/{$value['link']}?id=$obj->id'>";
                        print $obj->$key;
                        if (isset($value['link']))
                          print "</a>";
                        print "</td>";
